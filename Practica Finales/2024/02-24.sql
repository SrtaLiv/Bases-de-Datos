...
-- INSTALACION, idServ, Zona, nroC
-- CLIENTE fecha_alta, nroC,
-- SERVICIO tipoServ

CREATE ASSERTION ck_clientes CHECK (
       NOT EXISTS(
...
SELECT 1
FROM CLIENTE
JOIN INSTALACION I on CLIENTE.zona = I.Zona
AND CLIENTE.nroC = I.NroC
JOIN SERVICIO s USING (idServ)
GROUP BY I.NroC
HAVING (
    SELECT count(*)
    FROM INSTALACION i2
    WHERE I.NroC = i2.NroC
    AND tipoServ = 'V'
           ) < 3
HAVING